...
from reboot.utils.config import check_config, add_args, config
...
class Gateway:    
    def __init__(self, host, port, api_token=""):
        ...
        self.app = FastAPI(
            ...
            description="FastAPI implementation for Reboot Subnet Gateway",
            version="1.0.0"
        )
    ...
    def setup_routes(self):
        ...
        async def task_status(
            ...
            try:
                ...
                task_dict = self.task_manager.task_to_dict(task)
    ...
    async def process_robot_action(self, actions: List[Dict]):
        ...
        try:
            ...
            selected_uids = random.sample(range(metagraph.n.item()), 3)
            
            bt.logging.info(f"Randomly selected miner UIDs: {selected_uids}")
...
if __name__ == '__main__':
    Gateway(host="0.0.0.0", port=8080).run()